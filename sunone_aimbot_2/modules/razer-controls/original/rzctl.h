...
namespace rzctl {
	...
	*/
	bool mouse_move(int x, int y, bool from_start_point = true);
	bool mouse_click(int up_down);
	bool keyboard_input(short key, int up_down);
	bool is_initialized();
	DWORD last_error_code();
	DWORD failed_send_count();
	void shutdown();

	/*
	* communicates to the device
	*/
	bool _impl_mouse_ioctl(MOUSE_IOCTL_STRUCT* pMi);
}
